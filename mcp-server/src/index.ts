...
server.registerTool(
  ...
  {
    description:
      "Opens the student onboarding form (App 1) as an embedded MCP app. The user fills in personal, academic, and income details. When they submit, App 1 calls fetch_opportunities internally.",
    _meta: {
      ui: {
...
server.registerResource(
  "schemes-dashboard-ui",
  `${URI}/schemes-dashboard`,
  {
    mimeType: MIME,
...
server.registerTool(
  ...
  {
    description:
      "Called from inside App 1 after the student submits their profile. Runs the discover agent to find matching schemes. The structured result is sent back to the calling app via tool-input notification.",
    inputSchema: {
  ...
  async ({ profile }) => {
    ...
    return {
      content: [{ type: "text" as const, text: JSON.stringify({ profile, schemes }) }],
      structuredContent: { profile, schemes } as unknown as Record<string, unknown>,
    };
  }
);
...
server.registerTool(
  "show_schemes_dashboard",
  {
    description:
      "Opens the Schemes Dashboard (App 2). Pass the profile and schemes list so the dashboard can render. The app will receive data via ui/notifications/tool-input.",
    inputSchema: {
      profile: ProfileSchema,
      schemes: z.array(
        z.object({
...
server.registerResource(
  "doc-bundle-viewer-ui",
  `${URI}/doc-bundle-viewer`,
  {
    mimeType: MIME,
...
server.registerTool(
  ...
  {
    description:
      "Runs the verifier agent against a specific scheme and student profile. Returns a structured eligibility result.",
    inputSchema: {
      schemeId: z.string(),
      profile: z.object({
        state: z.string(),
...
server.registerTool(
  ...
  {
    description:
      "Runs the executor agent to build a document checklist for selected schemes. Returns the bundle data.",
    inputSchema: {
...
server.registerTool(
  ...
  {
    description:
      "Opens the Doc Bundle Viewer (App 3). The app receives data via ui/notifications/tool-input.",
    inputSchema: {
      bundle: z.object({
        studentId: z.string(),
